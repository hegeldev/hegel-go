...
func TestHandleTrackerUseAfterFree(t *testing.T) {
	h := newHandleTracker()
	h.register(settingsT(1), nil)     // owned root
	h.register(runT(1), settingsT(1)) // borrowed from the root
	...
	if !h.check(runT(1)) { // freed
		t.Error("freed handle not reported as use-after-free")
	}
}
