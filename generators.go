...
//
// Errors are recorded on the collection and surfaced via Err. If the server
// responds with a StopTest error (e.g. because too many rejections have
// exhausted the collection's budget), the recorded error stops further
// More() calls from issuing requests and propagates up through Err().
